"""
This module includes some CL scenarios
"""
from typing import Sequence, List, Optional, Dict, Any, Set

from avalanche.benchmarks import NCStream, NCExperience
from avalanche.benchmarks.scenarios.classification_scenario import ClassificationScenario
from avalanche.benchmarks.utils.classification_dataset import ClassificationDataset, SupervisedClassificationDataset
...
class NCScenario(
    ...
    """
    This class is identical to `avalanche.benchmarks.NCScenario` but in addition it adds a validation stream to the
    returned instance if a validation dataset is provided at construction time.

    The `train` and `test`streams can be accessed directly by using the `train_stream` and `test_stream` attributes of
    the returned instance, respectively. If a validation dataset is provided, the `val` stream can be accessed directly
    by using the `val_stream` attribute.
    ...
    `benchmarks.benchmarks_generators.nc_benchmark`.
    """
    ...
    def __init__(
            ...
            n_experiences: int,
            task_labels: bool,
            val_dataset: Optional[ClassificationDataset] = None,
            shuffle: bool = True,
        ...
        """
        Creates a ``NCGenericScenario`` instance given the training, test
        and (optionally) validation datasets and the number of experiences.
        ...
        :param val_dataset: The validation dataset. The dataset must be a
            subclass of :class:`AvalancheDataset`. For instance, one can
            use the datasets from the torchvision package like that:
            ``train_dataset=AvalancheDataset(torchvision_dataset)``. If None, no validation stream is added to the
        ...
        if val_dataset is not None:
            if not isinstance(val_dataset, SupervisedClassificationDataset):
        ...
        if self._has_task_labels:
            ...
            if val_dataset is not None:
                pattern_val_task_labels = ConstantSequence(0, len(val_dataset))
        ...
        for exp_id in range(n_experiences):
            ...
            if val_dataset is not None:
                selected_indexes_val = []
                for sc in selected_classes:
                    val_tgt_idx = original_val_dataset.targets.val_to_idx[sc]
                    selected_indexes_val.extend(val_tgt_idx)
                    if self._has_task_labels:
                        for idx in val_tgt_idx:
                            pattern_val_task_labels[idx] = exp_id

            train_exps_patterns_assignment.append(selected_indexes_train)
            test_exps_patterns_assignment.append(selected_indexes_test)
            if val_dataset is not None:
                val_exps_patterns_assignment.append(selected_indexes_val)
        ...
        if val_dataset is not None:
            val_dataset = classification_subset(
                val_dataset,
                class_mapping=self.class_mapping,
                initial_transform_group="eval",
            )
        ...
        if val_dataset is not None:
            self.val_exps_patterns_assignment = val_exps_patterns_assignment
            """ A list containing which validation instances are assigned to each
            experience in the val stream. Instances are identified by their id 
            w.r.t. the dataset found in the original_val_dataset field. """
        ...
        if val_dataset is not None:
            val_experiences: List[ClassificationDataset] = []
            val_task_labels: List[int] = []
            for t_id, exp_def in enumerate(val_exps_patterns_assignment):
                if self._has_task_labels:
                    val_task_labels.append(t_id)
                else:
                    val_task_labels.append(0)

                exp_task_labels = ConstantSequence(val_task_labels[-1], len(val_dataset))
        ...
        streams = {"train": (train_experiences, train_task_labels, train_dataset),
                   "test": (test_experiences, test_task_labels, test_dataset)}
        if val_dataset is not None:
            streams["val"] = (val_experiences, val_task_labels, val_dataset)
