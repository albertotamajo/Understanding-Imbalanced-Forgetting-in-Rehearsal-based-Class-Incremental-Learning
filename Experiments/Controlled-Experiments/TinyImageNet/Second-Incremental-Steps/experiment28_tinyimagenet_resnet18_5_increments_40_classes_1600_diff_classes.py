...
class-incremental learning setting consisting of 5 experiences of 40 classes each. Each training is performed with
1600 rehearsal samples. This module performs the second experience with different new classes starting from the same
first experience checkpoint produced by `experiment28_tinyimagenet_resnet18_5_increments_40_classes_1600.py`
"""
...
CLASS_ORDERS = [CLASS_ORDERS[0][:N_CLASSES_PER_EXP] + random.sample(CLASS_ORDERS[0][N_CLASSES_PER_EXP:],
                                                                    N_CLASSES_TOTAL - N_CLASSES_PER_EXP)
                for _ in range(N_CLASS_ORDERS)]
"""
The class orders where the first experience classes are all the same while the classes for the other experiences are
different
"""
...
CLASS_ORDERS = CLASS_ORDERS[:9]
EXP_SEEDS = EXP_SEEDS[:9]
REHEARSAL_SEEDS = REHEARSAL_SEEDS[:9]
...
if __name__ == "__main__":
    ...
    for index, (class_order, exp_seeds, rehearsal_seed) in enumerate(zip(CLASS_ORDERS, EXP_SEEDS, REHEARSAL_SEEDS)):
        ...
        runner.run(
            save_checkpoint_path=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_diff_classes",
            load_checkpoint=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_0_{N_EXEMPLARS}_experience_0.pth",
