...
with different new classes starting from the same second experience checkpoint produced by
...
if __name__ == "__main__":
    ...
    for index, (class_order, exp_seeds, rehearsal_seed) in enumerate(zip(CLASS_ORDERS, EXP_SEEDS, REHEARSAL_SEEDS)):
        ...
        runner.run(
            ...
            load_checkpoint=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_0_{N_EXEMPLARS}_experience_1.pth",
            ...
            plugins=[
                ReplaceStoragePolicy(
                    path=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_0_{N_EXEMPLARS}_storage_policy_experience_1.pth"
                )
            ],
