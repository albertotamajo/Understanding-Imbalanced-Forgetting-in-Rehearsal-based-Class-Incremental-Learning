...
setting consisting of 10 experiences of 10 classes each. It performs the third experience starting from the second
experience checkpoints produced by `experiment28_cifar100_resnet32_10_increments_10_classes_1000.py`.
"""
...
random.seed(16)  # set the random see for the creation of the splits in SplitCIFAR100; TODO needs to be different for
...
if __name__ == "__main__":
    ...
    for index, (class_order, exp_seeds, rehearsal_seed) in enumerate(zip(CLASS_ORDERS, EXP_SEEDS, REHEARSAL_SEEDS)):
        ...
        runner.run(
            save_checkpoint_path=f"experiment{EXP_ID}_cifar100_resnet32_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}",
            load_checkpoint=f"experiment{EXP_ID}_cifar100_resnet32_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_experience_1.pth",
            ...
            plugins=[
                ResizeBuffer(
                    checkpoint=f"experiment{EXP_ID}_cifar100_resnet32_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_experience_0.pth",
                    max_size=N_EXEMPLARS * 2,
                    experience=benchmark.train_stream[1],
                    save_path=f"experiment{EXP_ID}_cifar100_resnet32_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_storage_policy_experience_1.pth"
                )
            ],
