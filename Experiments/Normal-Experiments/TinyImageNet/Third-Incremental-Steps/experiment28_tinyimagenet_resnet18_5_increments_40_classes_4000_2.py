"""
This module allows performing training on the TinyImageNet dataset with an ImageNet ResNet-18 model in a
class-incremental learning setting consisting of 5 experiences of 40 classes each. It performs the third experience
starting from the second experience checkpoints produced by
`experiment28_tinyimagenet_resnet18_5_increments_40_classes_4000.py`.
"""
...
from benchmarks.classic import SplitTinyImageNet
...
random.seed(420)  # set the random see for the creation of the splits in SplitTinyImageNet; TODO needs to be different
...
TRAIN_TRANSFORM = transforms.Compose(
    [
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=torch.tensor([0.4914, 0.4822, 0.4465]),
                             std=torch.tensor([0.2023, 0.1994, 0.2010]))
    ]
)
"""
The transformations to be used during training.
They are similar to the transformations used in https://arxiv.org/abs/2302.03648
"""
...
EVAL_MB_SIZE = 9000
"""
The evaluation mini-batch size. It is set to 9000 because the number of training samples in each experience is 9000.
"""
...
def resnet18_tinyimagenet():
    ...
    # replace the ImageNet stem (7×7, s=2 + maxpool) with a small-image stem.
    model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    model.maxpool = nn.Identity()
    # replace the final avgpool with a sequence where the avgpool is performed, producing a feature of 512 dimensions,
    # and this feature is then fed into an MLP that maps the feature to 64 dimensions. The last feature is then fed into
    # the linear classifier
    model.avgpool = nn.Sequential(nn.AdaptiveAvgPool2d(output_size=(1, 1)),
...
class ResizeBuffer(RunnerPlugin):
    def __init__(self, checkpoint: str, max_size: int, experience, save_path: str):
        super().__init__()
        ...
        replay_plugin: ReplayNoOutlierPluginDynamicNetworkInitFeatureExtractorIncrementalOutlierTemplate = strategy.plugins[2]
        self.storage_policy = replay_plugin.storage_policy
        self.storage_policy.resize(None, max_size)
    ...
    def after_loading_checkpoint(self, runner: Runner, *args, **kwargs):
        ...
        new_data = self.experience.dataset.train()
        new_data = new_data.update_data_attribute("targets_task_labels", [0 for _ in range(len(new_data))])
        self.storage_policy.update_from_dataset(new_data, None)
        buffer_datasets = self.storage_policy.buffer_datasets
        print(f"Buffer datasets: {[len(b) for b in buffer_datasets]}")
        torch.save(self.storage_policy, self.save_path, pickle_module=dill)
...
if __name__ == "__main__":
    ...
    for index, (class_order, exp_seeds, rehearsal_seed) in enumerate(zip(CLASS_ORDERS, EXP_SEEDS, REHEARSAL_SEEDS)):
        ...
        benchmark = SplitTinyImageNet(
            ...
            dataset_root="/iridisfs/scratch/at2n19/DynamicNet-with-Incremental-Classifier-Outlier-Subnetworks/tinyimagenet",
            include_indices=True
        )

        strategy = DynamicNetworkInitFeatureExtractorIncrementalOutlierTemplate(
            # a single ResNet18 model adapted to tinyimagenet with a dynamic output layer for the class
            # incremental setting
            ...
            plugins=[
                ...
                ReplayNoOutlierPluginDynamicNetworkInitFeatureExtractorIncrementalOutlierTemplate(
                    storage_policy=ClassBalancedBufferDynamicNetworkInitFeatureExtractorIncrementalOutlierTemplate(
                        ...
                        # the first seed in `seeds` is used for seeding the creation of the replay buffer for each
                        # class
                        seed=rehearsal_seed
                    )
                )
        ...
        runner.run(
            save_checkpoint_path=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}",
            load_checkpoint=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_experience_1.pth",
            ...
            plugins=[
                ResizeBuffer(
                    checkpoint=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_experience_0.pth",
                    max_size=N_EXEMPLARS * 2,
                    experience=benchmark.train_stream[1],
                    save_path=f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS}_increments_{N_CLASSES_PER_EXP}_classes_{index}_{N_EXEMPLARS}_storage_policy_experience_1.pth"
                )
            ],
