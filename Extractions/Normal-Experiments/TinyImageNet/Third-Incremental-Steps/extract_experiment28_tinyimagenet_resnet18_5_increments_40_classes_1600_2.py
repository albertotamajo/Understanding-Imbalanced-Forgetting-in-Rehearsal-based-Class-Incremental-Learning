...
`experiment28_tinyimagenet_resnet18_5_increments_40_classes_1600_2.py`
"""
...
STORAGE_POL_SEC_EXP_PATH = (f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}" +
                           "_classes_{index_class_order}_{n_exemplars}_storage_policy_experience_1.pth")
"""
Path to the storage policy constructed at the end of the second experience
"""

SAVE_PATH = f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}_classes_{N_EXEMPLARS}_data_2.pkl"
"""
    ...
    for index_class_order in range(N_CLASS_ORDERS):
        for index_seed in range(N_SEEDS):
            for index_exp in range(N_EXPS):
                ...
                if index_exp == 1:
                    storage_policy = torch.load(STORAGE_POL_SEC_EXP_PATH.format(index_class_order=index_class_order,
