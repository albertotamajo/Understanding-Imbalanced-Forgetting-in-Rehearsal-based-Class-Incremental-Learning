...
`experiment28_tinyimagenet_resnet18_5_increments_40_classes_8000_diff_classes_2.py`
"""
...
N_EXEMPLARS_THIRD_EXP = N_EXEMPLARS * 2
"""
Number of exemplars in the rehearsal buffer during the third experience
"""
...
GRADIENT_SIZE_THIRD_EXP = (N_FEATURES * N_CLASSES_PER_EXP * 3) + (N_CLASSES_PER_EXP * 3)
"""
Size of the final-layer gradient during the third experience; it includes the number of parameters in the final-layer
weight matrix + the number of bias elements.
"""

CHECKPOINT_PATH = (f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}" +
                   f"_classes_0_{N_EXEMPLARS}"+"_experience_{index_exp}.pth")
"""
Path of the checkpoints of the experiments for the first class order; only the first and second incremental steps
"""

CHECKPOINT_THIRD_EXP_PATH = (f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}" +
                             "_classes_{index_class_order}_{n_exemplars}_diff_classes_experience_2.pth")
"""
Path of the checkpoints of the experiments for the third experience
"""

STORAGE_POL_SEC_EXP_PATH = (f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}" +
                            f"_classes_0_{N_EXEMPLARS}_storage_policy_experience_1.pth")
"""
Path to the storage policy constructed at the end of the second experience for the first class order
"""

SAVE_PATH = f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}_classes_{N_EXEMPLARS}_diff_classes_data_2.pkl"
"""
...
training forgetting of class j at the end of the current experience relative to its original training accuracy for the
ith seed. The ith, jth element in the second numpy array is the test forgetting of class j
at the end of the current experience relative to its original test accuracy for the ith seed. 
"""

exemplar_indices: Dict[int, List[np.ndarray]] = {i: [np.zeros((N_SEEDS,
                                                               N_EXEMPLARS if j == 0 else N_EXEMPLARS_THIRD_EXP),
                                                              dtype=np.int64)
                                                     for j in range(N_EXPS)]
                                                 for i in range(N_CLASS_ORDERS)}
"""
...
seeds and C is equal to the number of exemplars in the rehearsal buffer after the given experience. The ith row in each numpy
array contains the indices of all the samples selected for rehearsal at the end of the current experience for the ith
seed.
"""

avg_gradients_full_data: Dict[int, np.ndarray] = {i: np.zeros((TRAIN_EPOCHS + 1, N_SEEDS, N_CLASSES_PER_EXP * 3,
                                                               GRADIENT_SIZE_THIRD_EXP))
                                                  for i in range(N_CLASS_ORDERS)}
"""
a dictionary containing the class order indexes as keys and respective arrays as values. Each array is a 4D numpy array
of size ExNxCxL, where E is equal to the number of training epochs for the third experience +1---this is because it
contains one element computed prior to perform training on the third experience----N is equal to the number of seeds,
C is equal to the total number of classes during the third experience and L is equal to the size of the avg gradient
computed for the given class during the third experience for the given epoch and seed using the full training dataset
of the given class
"""

avg_gradients_replay_data: Dict[int, np.ndarray] = {i: np.zeros((TRAIN_EPOCHS + 1, N_SEEDS, N_CLASSES_PER_EXP*2,
                                                                 GRADIENT_SIZE_THIRD_EXP))
                                                    for i in range(N_CLASS_ORDERS)}
"""
a dictionary containing the class order indexes as keys and respective arrays as values. Each array is a 4D numpy array
of size ExNxCxL, where E is equal to the number of training epochs for the third experience +1---this is because it
contains one element computed prior to perform training on the third experience---N is equal to the number of seeds,
C is equal to the number of old classes during the third experience and L is equal to the size of the avg gradient
computed for the given class during the third experience for the given epoch and seed using the replay training dataset
of the given class
"""

theory_measures: Dict[int, np.ndarray] = {i: np.zeros((N_THEORY_MEASURES, TRAIN_EPOCHS + 1, N_SEEDS, N_CLASSES_PER_EXP*2))
                                          for i in range(N_CLASS_ORDERS)}
"""
a dictionary containing the class order indexes as keys and respective arrays as values. Each array is a 4D numpy array
of size TxExNxC, where T is equal to the number of theoretical measures derived from the theoretical analysis (11),
E is equal to the number of training epochs for the third experience +1---this is because it
contains one element computed prior to perform training on the third experience---N is equal to the number of seeds,
C is equal to the number of old classes during the third experience.
...
The ith, jth element in each array is the average logit for class j computed across all training
samples in the current experience (only the new classes) for the ith seed prior to perform training on the current
experience.
"""
...
def fill_forgetting_from_accuracy():
    for index_class_order, accuracies in accuracy.items():
        for n0, (train_acc_0, test_acc_0) in enumerate(accuracies):
            start_index = N_CLASSES_PER_EXP * n0
            end_index = N_CLASSES_PER_EXP * (n0 + 1)
            for n1, (train_acc_1, test_acc_1) in enumerate(accuracies[n0+1:]):
                n1 = n1 + n0 + 1
                train_forgetting, test_forgetting = forgetting[index_class_order][n1 - 1]
                train_forgetting[:, start_index: end_index] = train_acc_0[:, start_index: end_index] - train_acc_1[:, start_index: end_index]
                test_forgetting[:, start_index: end_index] = test_acc_0[:, start_index: end_index] - test_acc_1[:, start_index: end_index]
            ...
            for seed in range(N_SEEDS):
                ...
                for t in range(N_CLASSES_PER_EXP*2):
                    ...
                    class_t_avg_grad = np.concatenate((
                        ...
                        class_avg_grad[N_FEATURES*N_CLASSES_PER_EXP*3:][t:(t+1)]
                    ))
                    ...
                    sum_others_bias = bias[np.arange(N_CLASSES_PER_EXP*2) != t].sum(axis=0)
                    t_sum_others_bias = np.concatenate((
                        sum_others_bias[N_FEATURES * t: N_FEATURES * (t+1)],
                        sum_others_bias[N_FEATURES*N_CLASSES_PER_EXP*3:][t:(t+1)]
                    ))
                    ...
                    sum_old_class_gradients = avg_full_gradients[:N_CLASSES_PER_EXP*2].sum(axis=0)
                    t_sum_old_class_gradients = np.concatenate((
                        sum_old_class_gradients[N_FEATURES * t: N_FEATURES * (t + 1)],
                        sum_old_class_gradients[N_FEATURES * N_CLASSES_PER_EXP*3:][t:(t + 1)]
                    ))
                    ...
                    sum_old_class_gradients_except_self = (avg_full_gradients[:N_CLASSES_PER_EXP*2][
                        np.arange(N_CLASSES_PER_EXP*2) != t]).sum(axis=0)
                    t_sum_old_class_gradients_except_self = np.concatenate((
                        sum_old_class_gradients_except_self[N_FEATURES * t: N_FEATURES * (t + 1)],
                        sum_old_class_gradients_except_self[N_FEATURES * N_CLASSES_PER_EXP*3:][t:(t + 1)]
                    ))
                    ...
                    sum_new_class_gradients = avg_full_gradients[N_CLASSES_PER_EXP*2:].sum(axis=0)
                    t_sum_new_class_gradients = np.concatenate((
                        sum_new_class_gradients[N_FEATURES * t: N_FEATURES * (t + 1)],
                        sum_new_class_gradients[N_FEATURES * N_CLASSES_PER_EXP*3:][t:(t + 1)]
                    ))
    ...
    for index_class_order in range(N_CLASS_ORDERS):
        for index_seed in range(N_SEEDS):
            for index_exp in range(N_EXPS):
                ...
                if index_exp in [0, 1]:
                    # load checkpoint
                    ckp = torch.load(CHECKPOINT_PATH.format(index_exp=index_exp),
                                     pickle_module=dill, map_location="cpu")
                else:
                    # load checkpoint
                    ckp = torch.load(CHECKPOINT_THIRD_EXP_PATH.format(index_class_order=index_class_order,
                ...
                if index_exp == 1:
                    storage_policy = torch.load(STORAGE_POL_SEC_EXP_PATH, pickle_module=dill, map_location="cpu")
                else:
