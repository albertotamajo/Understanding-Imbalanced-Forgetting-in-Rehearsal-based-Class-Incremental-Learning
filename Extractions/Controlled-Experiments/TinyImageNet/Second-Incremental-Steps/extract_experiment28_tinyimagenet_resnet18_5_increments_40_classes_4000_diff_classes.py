...
`experiment28_tinyimagenet_resnet18_5_increments_40_classes_4000_diff_classes.py`
"""
...
CHECKPOINT_FIRST_EXP = (f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}"
                        f"_classes_0_{N_EXEMPLARS}_experience_0.pth")
"""
Checkpoint for the first experience; it is the same for all experiments
"""

CHECKPOINT_PATH = (f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}" +
                   "_classes_{index_class_order}_{n_exemplars}_diff_classes_experience_1.pth")
"""
Path of the checkpoints for the second experience of the experiments
"""

SAVE_PATH = f"experiment{EXP_ID}_tinyimagenet_resnet18_{N_EXPS_TOTAL}_increments_{N_CLASSES_PER_EXP}_classes_{N_EXEMPLARS}_diff_classes_data.pkl"
"""
    ...
    for index_class_order in range(N_CLASS_ORDERS):
        for index_seed in range(N_SEEDS):
            for index_exp in range(N_EXPS):
                ...
                if index_exp == 0:
                    # load checkpoint
                    ckp = torch.load(CHECKPOINT_FIRST_EXP, pickle_module=dill, map_location="cpu")
                else:
                    # load checkpoint
                    ckp = torch.load(CHECKPOINT_PATH.format(index_class_order=index_class_order,
